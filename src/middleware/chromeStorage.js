...
import { getState, onChange, setState } from '../chromeStorage'
...
const chromeStorage = store => {
	onChange(changes => {
		...
		if (
			changes.saveSelected &&
			saveSelected !== changes.saveSelected.newValue
		) {
			store.dispatch(
				setSaveSelected({ saveSelected: changes.saveSelected.newValue })
			)
		}
	})
	getState().then(state => {
		store.dispatch(
			setSaveSelected({ saveSelected: state.saveSelected || false })
		)
		if (state.tabGroups) {
				...
				case ADD_TAB_GROUP:
					...
					if (action.addTo !== undefined && action.addTo !== -1)
						setState({
							tabGroups: store
								.getState()
								.tabGroupList.tabGroups.map((tabGroup, tabGroupKey) => {
